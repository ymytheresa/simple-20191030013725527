...
class AtmsApiTestOAuth : OAuthBaseTest() {
    ...
    fun `should accept if client_credentials grant is used`() {
        val response = api.bankInfoAtmsGet().execute()
    ...
    fun `should reject if authorization_code grant is used`() {
        val response = apiAuthorizationCodeGrant.bankInfoAtmsGet().execute()
    ...
    fun `should reject if no OAuth is used`() {
        val response = apiNoAuth.bankInfoAtmsGet().execute()
