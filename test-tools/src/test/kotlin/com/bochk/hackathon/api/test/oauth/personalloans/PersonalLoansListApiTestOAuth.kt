...
class PersonalLoansListApiTestOAuth : OAuthBaseTest() {
    ...
    fun `should accept if authorization_code grant is used`() {
        val response = api.personalLoansGet().execute()
    ...
    fun `should reject if client_credentials grant is used`() {
        val response = apiClientCredentialsGrant.personalLoansGet().execute()
    ...
    fun `should reject if no OAuth is used`() {
        val response = apiNoAuth.personalLoansGet().execute()
